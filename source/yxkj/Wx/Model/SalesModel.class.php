        ...
        if($role_id == 9){
            ...
            if ($where['h.provice']) {
                $data['pro_num'] = M(self::TABLENAME_MONY)
                    ->alias('l')
                    ->join('left join ' . C('DB_PREFIX') . self::TABLENAME_HOTEL . ' as h on h.id = l.h_id')
            ...
            $data['type_num'] = M(self::TABLENAME_MONY)
                ->alias('l')
                ->join('left join ' . C('DB_PREFIX') . self::TABLENAME_HOTEL . ' as h on h.id = l.h_id')
                ->join('left join ' . C('DB_PREFIX') . self::TABLENAME_GET . ' as s on s.h_id = h.id')
                ->join('left join ' . C('DB_PREFIX') . self::TABLENAME_TYPE . ' as j on j.id = h.ht_id')
                ->join('left join ' . C('DB_PREFIX') . UserRoleModel::TABLENAME_USER . ' as x on x.id = s.sale_id')
                ->join('left join ' . C('DB_PREFIX') . \Pc\Model\SalesModel::TANLENAME_EXT . ' as p on p.u_id = x.id')
                ->where($where)
                ->where(array('s.is_default' => 1, 'l.price' => array('neq', 0)))
            ...
            $data['qd_num'] = M(self::TABLENAME_MONY)
                ...
                ->join('left join ' . C('DB_PREFIX') . self::TABLENAME_HOTEL . ' as h on h.id = l.h_id')
                ->join('left join ' . C('DB_PREFIX') . self::TABLENAME_GET . ' as s on s.h_id = h.id')
                ->join('left join ' . C('DB_PREFIX') . self::TABLENAME_USER . ' as j on j.id = s.sale_id')
                ->join('left join ' . C('DB_PREFIX') . \Pc\Model\SalesModel::TANLENAME_EXT . ' as p on p.u_id = j.id')
                ->where($where)
                ->where(array('s.is_default' => 1, 'l.price' => array('neq', 0)))
                ->where(array('l.status' => array('eq', 3)))
                ->where(array('s.sale_id' => array('neq', 0)))
            ...
            if ($where['h.provice']) {
                $data['bj'] = M(self::TABLENAME_MONY)
                    ...
                    ->join('left join ' . C('DB_PREFIX') . \Pc\Model\SalesModel::TANLENAME_EXT . ' as p on p.u_id = j.id')
                    ->where($where)
                    ->where(array('s.is_default' => 1, 'l.status' => array('eq', 3), 'l.price' => array('neq', 0)))
                    ->where(array('s.sale_id' => array('neq', 0)))
                    ->field('SUM(l.price) as price,h.provice')
                    ->group('provice')->select();
                $data['bj_nums'] = array();
            ...
            if ($where['l.ctime']) {
                $data['all_num'] = M(self::TABLENAME_MONY)
                    ...
                    ->join('left join ' . C('DB_PREFIX') . \Pc\Model\SalesModel::TANLENAME_EXT . ' as p on p.u_id = j.id')
                    ->join('left join ' . C('DB_PREFIX') . \Pc\Model\SalesModel::TABLENAME_CHL . ' as e on e.id = p.cl_id')
                    ->where($where)
                    ->where(array('s.is_default' => 1, 'l.price' => array('neq', 0), 'l.status' => 3))
                    ->where(array('s.sale_id' => array('neq', 0)))
            ...
            $data['all_num'] = arraySequence(self::toArrayList($data['all_num'], self::getMonth(), 'price'), 'months', 'SORT_ASC');
            // 二维数组排序,生成前十统计列表
            ...
            if ($where['h.provice']) {
                $data['pro_num'] = M(self::TABLENAME_MONY)
                    ...
                    ->field('SUM(l.price) as price,h.city')
                    ->where($where)
                    ->where(array('s.is_default' => 1, 'l.price' => array('neq', 0), 'x.parent_id' => $id))
                    ->where(array('l.status' => array('eq', 3)))
                    ->where(array('s.sale_id' => array('neq', 0)))
                    ->group('city')->select();
                // 处理所需要的业务数据
            ...
            $data['qd_num'] = M(self::TABLENAME_MONY)
                ...
                ->join('left join ' . C('DB_PREFIX') . \Pc\Model\SalesModel::TANLENAME_EXT . ' as p on p.u_id = j.id')
                ->where($where)
                ->where(array('s.is_default' => 1, 'l.price' => array('neq', 0), 'j.parent_id' => $id))
                ->where(array('l.status' => array('eq', 3)))
                ->where(array('s.sale_id' => array('neq', 0)))
                ->field('SUM(l.price) as price,(CASE WHEN p.channel_type = 1 THEN "内聘" '
                    . ' WHEN p.channel_type = 2 THEN "个人" ELSE "团队" END) as user_type_name')
            ...
            if ($where['h.provice']) {
                $data['bj'] = M(self::TABLENAME_MONY)
                    ...
                    ->where($where)
                    ->where(array('s.is_default' => 1, 'l.status' => array('eq', 3), 'l.price' => array('neq', 0)))
                    ->where(array('s.sale_id' => array('neq', 0), 'j.parent_id' => $id))
                    ->field('SUM(l.price) as price,h.provice')
                    ->group('provice')->select();
                $data['bj_nums'] = array();
                foreach ($data['bj'] as $keyfs => $valfs) {
            ...
            $rank = M(self::TABLENAME_MONY)
                ...
                ->join('left join ' . C('DB_PREFIX') . \Pc\Model\SalesModel::TANLENAME_EXT . ' as p on p.u_id = j.id')
                ->where($where)
                ->where(array('s.is_default' => 1, 'l.status' => 3, 'l.price' => array('neq', 0), 'j.parent_id' => $id))
                ->where(array('s.sale_id' => array('neq', 0)))
                ->where(array('l.status' => array('eq', 3)))
                ->field('SUM(l.price) as price,(CASE WHEN p.channel_type = 1 THEN "内聘" '
                    . ' WHEN p.channel_type = 2 THEN "个人" ELSE "团队" END) as user_type_name,j.real_name as real_name,j.img')
            ...
            if ($where['l.ctime']) {
                $data['all_num'] = M(self::TABLENAME_MONY)
                    ...
                    ->join('left join ' . C('DB_PREFIX') . \Pc\Model\SalesModel::TABLENAME_CHL . ' as e on e.id = p.cl_id')
                    ->where($where)
                    ->where(array('s.is_default' => 1, 'l.price' => array('neq', 0), 'l.status' => 3, 'j.parent_id' => $id))
                    ->where(array('s.sale_id' => array('neq', 0)))
                    ->field("SUM(l.price) as price,FROM_UNIXTIME(l.ctime,'%Y-%m') as months")
                    ->group('months')->select();
            } else {
...
//        $where['x.parent_id'] = $id;
     ...
     * 统计酒店
     */
    public static function countHotelNum($where)
    {
//        $id=session('USERINFO.id');
//        $where['h.parent_id'] = $id;
        ...
        $where['l.sale_id'] = array('neq', 0);
        $data = array();
        // 统计各省酒店占比
        $data['pro_num'] = M(\Pc\Model\HotelGetModel::TABLENAME)
            ->alias('l')
            ...
            ->where($where)
//            ->where(array('l.is_default' => 1, 's.is_sign' => 1))
//            ->where(array('l.sale_id'=>array('neq',0)))
            ->field("COUNT(s.id) as num,provice")
            ->group('provice')->select();
        $data['pro_num'] = arraySequence($data['pro_num'], 'num');
        // 处理所需要的业务数据
        $data['pro_nums'] = array();
        ...
        $data['pro_nums'] = json_encode($data['pro_nums']);
        // 统计酒店类型占比
        $data['type_num'] = M(\Pc\Model\HotelGetModel::TABLENAME)
            ->alias('l')
            ->join('left join ' . C('DB_PREFIX') . \Pc\Model\HotelModel::TABLENAME_HOTEL . ' as s on s.id = l.h_id')
            ->join('left join ' . C('DB_PREFIX') . \Pc\Model\HotelModel::TABLENAME_TYPE . ' as z on z.id = s.ht_id')
            ->join('left join ' . C('DB_PREFIX') . UserRoleModel::TABLENAME_USER . ' as h on h.id = l.sale_id')
            ->join('left join ' . C('DB_PREFIX') . \Pc\Model\SalesModel::TANLENAME_EXT . ' as p on p.u_id = h.id')
            ->where($where)
//            ->where(array('l.is_default' => 1, 's.is_sign' => 1))
//            ->where(array('l.sale_id'=>array('neq',0)))
            ->field('COUNT(s.id) as num,z.name as type_name')
            ->group('type_name')->select();
        $data['type_num'] = arraySequence($data['type_num'], 'num');
        // 处理所需要的业务数据
        $data['type_nums'] = array();
        foreach ($data['type_num'] as $keys => $vals) {
        ...
        $data['type_nums'] = json_encode($data['type_nums']);
        // 各渠道占比
        $data['qd_num'] = M(\Pc\Model\HotelGetModel::TABLENAME)
            ->alias('l')
            ->join('left join ' . C('DB_PREFIX') . \Pc\Model\HotelModel::TABLENAME_HOTEL . ' as s on s.id = l.h_id')
            ->join('left join ' . C('DB_PREFIX') . UserRoleModel::TABLENAME_USER . ' as h on h.id = l.sale_id')
            ->join('left join ' . C('DB_PREFIX') . \Pc\Model\SalesModel::TANLENAME_EXT . ' as p on p.u_id = h.id')
...
//            ->where(array('l.is_default' => 1, 's.is_sign' => 1))
//            ->where(array('l.sale_id'=>array('neq',0)))
            ->field('COUNT(s.id) as count,(CASE WHEN p.channel_type = 1 THEN "内聘" '
                . ' WHEN p.channel_type = 2 THEN "个人" ELSE "团队" END) as user_type_name')
            ->group('user_type_name')->select();
        // 处理所需要的业务数据
        $data['qd_nums'] = array();
        ...
        $data['all_num'] = M(\Pc\Model\HotelGetModel::TABLENAME)
            ->alias('l')
            ->join('left join ' . C('DB_PREFIX') . \Pc\Model\HotelModel::TABLENAME_HOTEL . ' as s on s.id = l.h_id')
            ->join('left join ' . C('DB_PREFIX') . UserRoleModel::TABLENAME_USER . ' as h on h.id = l.sale_id')
            ->join('left join ' . C('DB_PREFIX') . \Pc\Model\SalesModel::TANLENAME_EXT . ' as p on p.u_id = h.id')
...
//            ->where(array('l.sale_id'=>array('neq',0)))
            ...
            ->group('months')->select();
        $data['all_num'] = arraySequence(self::toArrayList($data['all_num'], self::getMonth(), 'num'), 'months', 'SORT_ASC');
        // 统计优秀销售员排行榜
        $rank = M(\Pc\Model\HotelGetModel::TABLENAME)
            ->alias('l')
            ->join('left join ' . C('DB_PREFIX') . \Pc\Model\HotelModel::TABLENAME_HOTEL . ' as s on s.id = l.h_id')
            ->join('left join ' . C('DB_PREFIX') . UserRoleModel::TABLENAME_USER . ' as h on h.id = l.sale_id')
            ->join('left join ' . C('DB_PREFIX') . \Pc\Model\SalesModel::TANLENAME_EXT . ' as p on p.u_id = h.id')
...
//            ->where(array('l.is_default' => 1, 's.is_sign' => 1))
//            ->where(array('l.sale_id'=>array('neq',0)))
            ->field('COUNT(s.id) as num,(CASE WHEN p.channel_type = 1 THEN "内聘" '
                . ' WHEN p.channel_type = 2 THEN "个人" ELSE "团队" END) as user_type_name,h.real_name as real_names,h.img')
            ->group('real_names')->select();
        $ranks = array_sum(i_array_column($rank, 'num'));
        foreach ($rank as $keyk => &$valk) {
            $valk['bai'] = ($valk['num'] / $ranks) * 100;
        }
        // 二维数组排序,生成前十统计列表
    ...
    public static function countHotelRoom($where)
    {
        ...
        $data = array();
        // 统计房间类型占比量
        $data['type_room_num'] = M(\Pc\Model\HotelModel::TABLENAME_ROOMS)
            ->alias('l')
        ...
        $map['l.ctime'] = array('lt', $nowtime);
        $data['hotel_room'] =
            M(\Pc\Model\HotelModel::TABLENAME_ROOMS)
                ->alias('l')
                ->join('left join ' . C('DB_PREFIX') . \Pc\Model\HotelModel::TABLENAME_ROOMSE . ' as s on s.rt_id = l.id')
                ->join('left join ' . C('DB_PREFIX') . \Pc\Model\HotelModel::TABLENAME_HOTEL . ' as j on j.id = s.h_id')
                ->join('left join ' . C('DB_PREFIX') . \Pc\Model\HotelModel::TABLENAME_GET . ' as e on e.h_id = j.id')
                ...
                ->where(array('j.is_sign' => 1,'e.is_default'=>1))
                ->group('months')->select();
        $data['hotel_room'] = arraySequence(self::toArrayList($data['hotel_room'], self::getMonth(), 'num'), 'months', 'SORT_ASC');
        // 处理所需要的业务数据
    ...
    public static function countGoodsNum($where)
    {
        ...
        $e1_num = M(self::TABLENAME_CONT)
            ...
            ->where($where)
            ->where(array('e.is_default' => 1, 's.is_sign' => 1))
            ->group('name')->select();
    ...
    public static function countContract($where)
    {
        ...
        $data['canal'] = M(self::TABLENAME_CONT)
            ...
            ->join('left join ' . C('DB_PREFIX') . self::TABLENAME_HOTEL . ' as h on h.id = l.h_id')
            ->join('left join ' . C('DB_PREFIX') . self::TABLENAME_GET . ' as s on s.h_id = h.id')
            ->join('left join ' . C('DB_PREFIX') . self::TABLENAME_USER . ' as z on z.id = s.sale_id')
            ->join('left join ' . C('DB_PREFIX') . self::TABLENAME_EXT . ' as f on f.u_id = z.id')
            ->where($where)
            ->field('COUNT(l.id) as num,(CASE WHEN f.channel_type = 1 THEN "内聘" '
                . ' WHEN f.channel_type = 2 THEN "个人" ELSE "团队" END) as user_type_name')
            ->group('user_type_name')->select();
        // 全年趋势图
        $nowtime = strtotime(date("Y-m", strtotime("+1 months", strtotime(date("Y-m", time())))));
        if($where['l.ctime']){
        ...
        $rank = M(self::TABLENAME_CONT)
            ...
            ->where($where)
            ->field('COUNT(l.id) as num,(CASE WHEN f.channel_type = 1 THEN "内聘" '
                . ' WHEN f.channel_type = 2 THEN "个人" ELSE "团队" END) as user_type_name,j.real_name as real_name,z.img')
            ->group('real_name')->select();
        $ranks = array_sum(i_array_column($rank, 'num'));
        foreach ($rank as $keyk => &$valk) {
            $valk['bai'] = ($valk['num'] / $ranks) * 100;
    ...
    public static function countChannel($where)
    {
        ...
        if ($where['p.channel_type']) {
            ...
            $data['all_num_one'] = \Pc\Model\SalesModel::toArrayList($data['all_num_one'], self::getMonth(), 'num');
            ...
            if($where['l.ctime']){
                ...
                $data['all_num_two'] = M(UserRoleModel::TABLENAME_USER)
                    ...
                    ->group('months')->select();
                $data['all_num_two'] = \Pc\Model\SalesModel::toArrayList($data['all_num_two'], self::getMonth(), 'num');
            ...
            $data['all_num_the'] = \Pc\Model\SalesModel::toArrayList($data['all_num_the'], self::getMonth(), 'num');
     ...
     * php获取以前的十二个月(包含本月)
     */
    public static function getMonth($num = 6)
    {
        $temp = array();
        $nowtime = strtotime(date("Y-m", strtotime("+1 months", strtotime(date("Y-m", time())))));
        while ($num > 0) {
