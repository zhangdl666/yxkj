...
class BaseController extends Controller{
	...
	public function _initialize(){
		...
		$this->assign('group_name','/Traveller/');
...
//        }
		/* 微信JS所需 */
		/*$wx_config = C('TWEIXIN');
        $jssdk = new JSSDK($wx_config['AppID']);
	...
	public function index(){
		...
		cookie('__forwardss__', $_SERVER['REQUEST_URI']);
    ...
    public function getubind($openid){
        ...
        $re_data = json_decode(Http::get($url, $params), true);
        //$re_data = $this->httpGet($geturl);
        ...
        //$re_data = json_decode($re_data);
        //file_put_contents('./ubfile.txt', var_export($re_data,true)."\r\n",FILE_APPEND);
        if($re_data['code'] == 200){
            //$information = array_slice($re_data['devices'],0,1);
            return $re_data['devices'];

//            foreach ($information as $key => $value) {
//                    $data['Mac'] = $value['mac'];
//                    $data['devices'] = $value['deviceid'];
//                    return $data;
//            }
        }else{
            return $re_data['message'];
        }
    }
	...
	public function saveWXFile($img_dir=null,$media_id=null,$num=0){
		...
		if(empty($curl_data->errcode) && $num < 5){
			++$num;
			$this->access_token();
