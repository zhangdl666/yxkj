...
class RunInfoModel extends BaseModel
{
    ...
    const MultipleDevices = "http://api.bjhike.com/DeviceData_Batch"; // 获取多个设备信息
    /*
        const GetDeviceInfo = "http://yxkj.firerise.com.cn/CreaData/deviceInfo"; // 获取设备信息(传入错误的mac地址,也会返回200,但是devices与message是为空的)
        const GetDeviceDay = "http://yxkj.firerise.com.cn/CreaData/days"; // 以天为单位的数据接口
        const getDeviceNewInfo = "http://yxkj.firerise.com.cn/CreaData/deviceData"; // 设备最新一次状态
        const GetDeviceHoures = "http://yxkj.firerise.com.cn/CreaData/hours"; // 以小时为单位的数据接口
        const GetDeviceAllday = "http://yxkj.firerise.com.cn/CreaData/device_alldata"; // 获取设备某时间段的信息
        const GetCityData = "http://api.bjhike.com/api/v2_weather"; //获取城市区域的天气
        const GetDevicesTime = "http://yxkj.firerise.com.cn/CreaData/deviceLoginLog"; // 获取时间锻
    */
    // 2.0接口(成功返回的状态吗)
    ...
    public static function getParamsSecv($mac)
    {
        $params = array(
            'CompanyToken' => self::Token,
            'DeviceMacs' => $mac,
    ...
    public static function getMultipleInfo($mac)
    {
        $params = self::getParamsSecv($mac);
        // 将返回的数据转换为数组格式
        $data = json_decode(Http::post(self::MultipleDevices, $params), true);
        // 请求的url
        return $data;
    }
...
//        $newdata = RunInfoModel::getNewDevicesInfo($mac);
        $devicedata = array(array('equipment_sno' => $mac));
        $strs = DeviceRunModel::DeviceString($devicedata);
        $Infos = RunInfoModel::getMultipleInfo($strs);
        $online = $Infos['设备数据'][0]['online'];
        $power = $Infos['设备数据'][0]['power'];
        $listArray['hours'] = self::listToDay($data, $online, $power);
     ...
     * @param $data
     */
    public static function listToDay($data, $online, $power)
    {
        // 检测当前设备是否是开启状态
        $type = end($data)['type'];
    ...
    public static function monthRunDay($data, $time)
    {
        ...
        // 判断最后一个数组里面的数据仪器是否关闭了,只要今天开启一小时以上,算一天
        $type = end($listData)['type'];
        if ($type == 1) {
            // 最后一个加入实时时间
            $num = count($listData);
        ...
        $listDatas = [];
        foreach ($listData as $key => $val) {
            $h = intval(date("H", $val['time']));
            if ($h > 0 && $h < 24) {
                if ((($val['time'] - $val[$key - 1]['time']) % 86400 / 3600) > 1) {
                    $val['times'] = date('Y-m-d', $val['time']);
                    $listDatas[] = $val;
                }
            }
        }
        $counts = SalesModel::listArrayKey($listDatas, 'times');
        return count($counts);
    }
...
//        dump($data);
        ...
        foreach ($data as $key => $val) {
            if ($data[$key + 1] && $data[$key + 1]['type'] == 1) {
                if ($val['time'] && $data[$key - 1]['time']) {
                    $allTime += ($val['time'] - $data[$key - 1]['time']) / 3600 / 24;
                }
            }
        }
